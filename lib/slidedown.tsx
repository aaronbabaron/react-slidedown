...
class SlideDownContent extends React.Component<SlideDownContentProps> {
  ...
  componentWillEnter(callback: Function) {
    this.callbacks.push(callback);
    const prevHeight =
      this.outerRef.current.getBoundingClientRect().height + "px";
  ...
  componentWillLeave(callback: Function) {
    ...
    this.outerRef.current.style.height = getComputedStyle(
  ...
  componentDidUpdate(_prevProps, _prevState, snapshot: string | null) {
    ...
    if (this.callbacks.length === 0) {
      const prevHeight =
        snapshot || getComputedStyle(this.outerRef.current).height;
      this.startTransition(prevHeight);
    }
  }
...
export const SlideDown = React.memo(
  React.forwardRef(
    (props: SlideDownProps, ref: React.RefObject<HTMLDivElement>) => (
